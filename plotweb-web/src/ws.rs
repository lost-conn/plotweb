//! Feedback live-update WebSocket client.
//!
//! The live-push transport is **web-only** for now: it uses `web_sys::WebSocket`
//! and `window.location`. On native the public functions are no-op stubs so the
//! frontend still compiles for desktop — live feedback push on native needs a
//! cross-platform WebSocket (tracked as an upstream `rinch-ws` card). Beta
//! readers / feedback are online-only anyway, so this is not on the offline path.

use plotweb_common::{BetaFeedback, BetaFeedbackReply};
use serde::{Deserialize, Serialize};

/// A message pushed over the feedback WebSocket. Cross-platform (plain serde);
/// only the transport is web-only.
...
pub use web::{connect_feedback_ws, ws_url};

#[cfg(not(target_arch = "wasm32"))]
pub use native_stub::{connect_feedback_ws, ws_url};

// ── Native stub ──────────────────────────────────────────────────────────────
// Live feedback push isn't wired on native yet (needs an upstream cross-platform
// WebSocket). These no-ops keep the call sites compiling for desktop.
#[cfg(not(target_arch = "wasm32"))]
mod native_stub {
    use super::WsMessage;

    /// No-op on native: live feedback push isn't available yet.
    pub fn connect_feedback_ws(_url: &str, _on_message: impl Fn(WsMessage) + 'static) {}

    /// On native there is no page location; return the path unchanged (the URL is
    /// only consumed by the no-op `connect_feedback_ws`).
...
mod web {
    ...
    /// `.forget()` (which leaked a fresh set of closures on every reconnect). When
    /// a new connection is established, the previous `WsConn` is dropped, releasing
    /// the old closures and socket.
    struct WsConn {
    ...
    /// Connect to a feedback WebSocket and call `on_message` for each incoming
    /// message. Automatically reconnects with exponential backoff on disconnection.
    pub fn connect_feedback_ws(url: &str, on_message: impl Fn(WsMessage) + 'static) {
        let url = url.to_string();
        let on_message = Rc::new(on_message);
        // Shared current-backoff cell: reset to the initial delay on every
        // successful open, and grown (exponential, capped) on each close.
        let backoff = Rc::new(Cell::new(1000));
        // Slot that owns the current connection's closures. Replacing its contents
        // on reconnect drops the previous closure set instead of leaking it.
    ...
    fn do_connect(
        ...
        // onopen — reset backoff to the initial delay so a brief blip doesn't
        // leave us stuck at the maximum reconnect interval.
    ...
    pub fn ws_url(path: &str) -> String {
        ...
        // If running on Trunk dev server (port 8080), route WS to API server (3000)
